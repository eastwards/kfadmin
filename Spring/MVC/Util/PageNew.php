...
class PageNew
{
	 ...
	 * 分页方法
	 */
	public function get($total, $pageRows = 20, $point = 10, $style = 'on', $func='')
	{
		$prior   = $point % 2 == 0 ? ceil($point/2) + 1 : ceil($point/2);
		$back    = ceil($point/2);
		...
		if ($page <= $back && ($page + $point-1 <=$num) )
		{
			for($j=1; $j<=$point; $j++)
			{
				if ( empty($func) ){
					...
					$link = $j == $page ? '<a href="javascript:;" class="'.$style.'">'.$j."</a> " : '<a href="javascript:;" onclick="'.$this->prefix.$j.$this->suffix.'">'.$j."</a>";
				}
			...
			for($i=1; $i<=$num; $i++)
			{
				if ($i < ($page + $back) && $i > ($page - $prior))
				{
					if ( empty($func) ){						
						...
						$point = $i == $page ? '<a href="javascript:;" class="'.$style.'">'.$i."</a>" : '<a href="javascript:;" onclick="'.$this->prefix.$i.$this->suffix.'">'.$i."</a>";
					}$listStr .= $point;
				}
			}
		}
		$result['point']= $listStr;
		$express = empty($func) ? 'eval(targ+".location=\''.$this->prefix.'"+jumpnum+"\'");' : $this->prefix.'jumpnum)';
		$result['jump']	= '
