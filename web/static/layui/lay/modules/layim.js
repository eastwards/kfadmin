...
layui.define(['layer', 'laytpl', 'upload'], function (exports) {
    ...
    var LAYIM = function () {
        ...
        $('body').on('click', '*[layim-event]', function (e) {
            ...
            events = $.extend(events, my_events || {});  //合并外部用户自定义的events
    ...
    var elemTpl = ['<div class="layui-layim-main">'
        ...
        , '<ul class="layui-layim-list layui-friend {{# if(spread === "true"){ }}'
        , ' layui-show'
        , '{{# } }}">'
        , listTpl({
            type: "friend"
        ...
        , '<li><ul class="layui-layim-list layui-friend layui-show"><li class="layim-null">暂无联系人</li></ul>'
        , '{{# } }}'
        , '</ul>'
    ...
    layui.data.content = function (content) {
        ...
        content = (content || '').replace(/&(?!#?[a-zA-Z0-9]+;)/g, '&amp;')
            ...
            }).replace(/audio\[([^\s\[\]]+?)\]/g, function (str) { //录音文件
                var audio = str.replace(/^audio/g, '');
                audio = audio.replace(/[\[\]]/g, '');
                return '<audio src="' + audio + '" controls="controls"></audio>';
    ...
    var banRightMenu = function () {
        ...
        //自定主面板联系人右键功能 --- NickBai,修改了主模板渲染 添加了 layui-friend 样式名
        layimMain.find('.layui-friend').on('contextmenu', 'li', function (e) {
            var othis = $(this);
            var html = '<ul data-id="' + othis[0].id + '" data-index="' + othis.data('index') + '">' +
                '<li layim-event="enterZone">进入空间</li><li layim-event="joinBlack">加入黑名单</li>' +
                '<li layim-event="changeGroup">移动分组</li><li layim-event="removeFriend">删除该好友</li>' +
                '<li layim-event="reportFriend">举报</li></ul>';
        ...
        // 自定义主面板 群组右键功能 --- NickBai layim-list-group
        ...
        if (window.Notification) {
            if (Notification.permission === 'granted') {
                var notification = new Notification(data.title || '', {
                    body: data.content || ''
                    , icon: data.avatar || 'http://xishanpo.com/static/images/default_avatar_male_180.gif'
                });
            } else {
    ...
    var anim = 'layui-anim-upbit', events = {
        ...
        , about: function () {
            ...
            layer.alert('<a href="/index/userzone/index" target="_blank">点击，进入我的空间</a>', {
                title: false
                , icon: 6
                , btn: ''
            });
